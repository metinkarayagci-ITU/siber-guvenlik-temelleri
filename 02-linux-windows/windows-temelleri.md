...
> **Kesişim — hash'i makinede olmadan çalmak (coerced authentication):** Hash elde etmek için her zaman makineyi ele geçirmek gerekmez. Windows bir `\\sunucu\paylasim` (UNC) yolu gördüğünde **otomatik ve şeffaf** olarak NTLM ile kimlik doğrulamaya çalışır. Saldırgan kurbanı, kontrol ettiği bir SMB sunucusuna (port 445) bağlanmaya kandırırsa (kötü bir link, belge, veya Outlook Moniker Link zafiyeti CVE-2024-21413 → [12-sosyal-muhendislik-phishing/phishing-analizi.md](../12-sosyal-muhendislik-phishing/phishing-analizi.md)), kurbanın NTLMv2 hash'i sunucuya sızar — kurban parola bile girmemiştir. Sonra offline kırma veya **NTLM relay** (hash'i başka bir sisteme yönlendirip o kimlikle giriş) devreye girer. Bu, phishing (sosyal mühendislik) ile kimlik hırsızlığının (Windows kimlik modeli) nasıl tek zincirde birleştiğini gösterir. Savunma: giden SMB'yi (445) dışarı engelle, NTLM'i kısıtla/Kerberos'a geç.
