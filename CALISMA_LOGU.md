...
- **Kablosuz (Wi-Fi) güvenlik ✅**: yeni `01/kablosuz-guvenlik.md` — WEP/WPA/WPA2/WPA3 evrimi, 4-way handshake yakalama (airmon/airodump/aireplay/aircrack), offline kırma (hashcat 22000), deauth (kimliksiz yönetim çerçevesi), evil twin/rogue AP, açık ağ dinleme, WPS/PMKID, WPA3/SAE. Simetrik: WPA offline kırma ↔ hash_kirma lab (05) [iki yönlü]; evil twin ↔ ARP zehirleme/MITM (01) + phishing captive portal (12); açık ağ ↔ TLS/VPN gerekliliği (05/01); WPA2-Enterprise ↔ 802.1X (06). Sözlük: WPA2/3, 4-way handshake/deauth, evil twin (3 terim).
- **XSS derinleştirme ✅** (SQLi ile paralel — kalibrasyon örneğinin kardeşi): `xss.md §3.5` — bağlama göre payload (HTML gövde/öznitelik/JS/href), filtre-WAF atlatma (event handler/svg/Base64/case), CSP atlatma temel mantığı, gerçek silahlandırma (çerez hırsızlığı, keylogger). Simetrik: Base64 atlatma ↔ SQLi WAF atlatma (04) + kodlama≠şifreleme (00); çerez hırsızlığı ↔ HttpOnly (01 http); XSS>CSRF ↔ csrf-ssrf (04).
- **Pivoting/tunneling ✅**: somuru-ve-sonrasi genişletildi (SSH local/dynamic forward, proxychains/SOCKS, chisel) — segmentasyon testi. Simetrik: pivoting ↔ segmentasyon (01) + zero-trust (06) + tünel tespiti (11).
- **Paket analizi (Wireshark/tcpdump) ✅** (core hacker becerisi, repo protokol teorisi vardı ama "paketi okuma" pratiği yoktu): yeni `01/pratik-lab/paket-analizi-wireshark.md` — tcpdump/Wireshark, capture vs display filter, 3-way handshake'i telde okuma, Follow TCP Stream ile düz metin kimlik bilgisi, ARP/DNS/DHCP anomali tespiti. Simetrik: handshake ↔ tcp-ip (01) [iki yönlü]; ARP zehirleme/MITM ↔ temel-kavramlar (01) [iki yönlü]; düz metin ↔ TLS gerekliliği (05); network forensics ↔ dijital-forensics (11) + malware dinamik analiz (11). Sözlük: Wireshark/tcpdump, pcap, sniffing, capture/display filter (4 terim).
